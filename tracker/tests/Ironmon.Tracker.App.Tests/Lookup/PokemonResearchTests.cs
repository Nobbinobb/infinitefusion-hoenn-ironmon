...
public sealed class PokemonResearchTests
{
    ...
    public async Task SwapRequiresActiveRunPermissionAndConfirmation(bool active, bool authorized)
    {
        await RenderAsync<PokemonLookupCard>(new()
        {
            ...
            [nameof(PokemonLookupCard.Redesigned)] = true,
    ...
    public async Task RedesignedSlotsOpenComponentDescriptionsWithoutDuplicatingIdentifiers()
    {
        ...
        await RenderAsync<DebugAbilitySlots>(new()
        {
            ...
            [nameof(DebugAbilitySlots.Redesigned)] = true,
    ...
    /// <summary>
    /// Preserves the diagnostic IDs still used by the unrefactored live inspector.
    /// </summary>
    /// <returns>A task representing legacy rendering.</returns>
    [Fact]
    public async Task LegacySlotsRetainTheirDiagnosticPresentation()
    {
        await RenderAsync<DebugAbilitySlots>(new()
        {
            [nameof(DebugAbilitySlots.Slots)] = new DebugAbilitySlotSnapshot[] { new() { AbilityId = _abilityId, AbilityName = _abilityName, OriginalAbilityId = _originalId, OriginalAbilityName = _originalName } }
        }, false, (_, html) =>
        {
            Assert.Contains(_abilityId, html());
            Assert.Contains(_originalId, html());
            Assert.DoesNotContain("research-ability-name", html());
