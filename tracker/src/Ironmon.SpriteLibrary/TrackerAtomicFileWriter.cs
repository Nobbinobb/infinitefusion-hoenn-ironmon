...
internal static class TrackerAtomicFileWriter
{
    ...
    internal static void WriteAllText(string path, string contents)
    {
        ...
        using var existingParents = OperatingSystem.IsWindows() ? WindowsUpdateAccess.HoldAncestors(path) : null;
        string temporaryPath = CreateTemporaryPath(path);
        try
        {
            Directory.CreateDirectory(GetDirectory(path));
            using var destinationParents = OperatingSystem.IsWindows() ? WindowsUpdateAccess.HoldAncestors(path) : null;
            using (var writer = new StreamWriter(new FileStream(temporaryPath, FileMode.CreateNew, FileAccess.Write, FileShare.None)))
    ...
    internal static async Task WriteAllTextAsync(string path, string contents, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            Directory.CreateDirectory(GetDirectory(path));
            using var destinationParents = OperatingSystem.IsWindows() ? WindowsUpdateAccess.HoldAncestors(path) : null;
            await using (var writer = new StreamWriter(new FileStream(temporaryPath, FileMode.CreateNew, FileAccess.Write, FileShare.None)))
    ...
    private static string CreateTemporaryPath(string path)
    {
        ...
        return Path.Combine(directory, $".{fileName}.{Guid.NewGuid():N}{TemporaryExtension}");
    }
