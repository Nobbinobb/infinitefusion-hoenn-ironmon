...
public partial class EnemyCard : IDisposable
{
    ...
    private Task SelectEnemy(string enemyId) => SelectedEnemyIdChanged.InvokeAsync(enemyId);
    ...
    private string GetName() => SelectedEnemy?.SpeciesName ?? "No opponent";
    ...
    private string GetBaseStatTotal() => SelectedEnemy?.BaseStatTotal.ToString(CultureInfo.InvariantCulture) ?? "--";
    ...
    private void HandleKnowledgeChanged(object? sender, EventArgs args) => _ = InvokeAsync(StateHasChanged);
    ...
    /// </summary>
    public void Dispose() => Knowledge.Changed -= HandleKnowledgeChanged;
}
