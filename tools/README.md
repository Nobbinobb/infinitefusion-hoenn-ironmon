...
ordered custom-fusion pool, type masks, compatibility fingerprints, explicit
autogenerated-sprite-author exclusions, and Ruby-reference mappings used by
the tracker's parallel obtainability worker. The release pipeline regenerates
the catalog before tracker tests, whose cross-runtime audit rejects any mapping
...
stop the release. See `docs/audits/OBTAINABILITY_FOUNDATION_AUDIT.md` for the
...
`analysis/Run-Obtainability-Prototype-Benchmark.ps1` measures the implemented
shared service's Ruby fallback for the fixed Classic, Mixed, random-component
scenario. It explicitly disables the tracker mapping worker so progress
snapshots cannot renew a worker handoff lease that the standalone benchmark
cannot fulfill. It
records recipe preparation separately from deferred service initialization,
cold foreground completion in 250 ms chunks, and a second cold pass through
every 4 ms background slice up to the foreground-only authored audit. Maximum
foreground and background phases are included so indivisible latency spikes
remain visible alongside total time.

`analysis/Run-Obtainability-Tracker-Batch-Benchmark.ps1` separates the
tracker-worker handoff from the game's remaining proof-plan work. It prepares
the numeric mapping fixture in one bundled-runtime process, then exits and
measures setup, foreground batch application, completion polling, a
representative 160-edge graph workload, compact batch
payload parsing, and 4 ms cooperative slices in a clean second process. The
foreground fixture uses 8,192-pair batches and one initial plus one post-batch
snapshot, matching the live tracker's drained mapping exchange. Every protocol
snapshot resubmits the graph edges just as the live tracker does. The excluded
Ruby fixture preparation is reported separately. Tracker transport, graph
rendering, and frame scheduling remain outside the boundary, so the result must
not be presented as complete UI latency.
