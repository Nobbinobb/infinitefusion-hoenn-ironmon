...
$reportPath = Join-Path $projectRoot 'data\defense-overview-test.json'
$loader = Get-Content (Join-Path $PSScriptRoot 'generation\Script-Loader.rb') -Raw
$testSource = @(
    Get-Content (Join-Path $projectRoot 'tests\runtime\Defense-Coverage.rb') -Raw
    Get-Content (Join-Path $projectRoot 'tests\runtime\Defense-Overview.rb') -Raw
) -join "`n"
$bootstrap = @(
    '# encoding: UTF-8'
    'begin'
    ...
    'GameData.load_all'
    ('$ironmon_defense_test_output_path = "' + $reportPath.Replace('\', '/') + '"')
    $testSource
    'exit! 0'
...
$report | Select-Object passed, assertions, abilities, items, sound_moves | ConvertTo-Json -Compress
