param(
    ...
    [string]$OutputPath = (Join-Path (Split-Path -Parent (Split-Path -Parent $PSScriptRoot)) "data\fusion_predecessor_index.dat"),
    [ValidateRange(1, 600)]
...
$exporterSourcePath = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot "Export-FusionPredecessorIndex.rb"))
...
Assert-PathWithinDirectory -Path $exporterSourcePath -Directory $resolvedGameRoot
foreach ($requiredPath in $loaderSourcePath, $exporterSourcePath, $resolvedSourceManifestPath) {
    if (-not (Test-Path -LiteralPath $requiredPath)) {
        throw "Required fusion predecessor index input was not found at '$requiredPath'."
    }
}
...
$exporterCode = [IO.File]::ReadAllText($exporterSourcePath, [Text.Encoding]::UTF8)
$bootstrapSource = "File.binwrite(`"$bootstrapMarker`", `"bootstrap loaded\n`")`n`$ironmon_fusion_predecessor_index_output_path = `"$rubyOutputPath`"`n`$ironmon_fusion_predecessor_index_game_root = `"$rubyGameRoot`"`n`$ironmon_fusion_predecessor_index_source_path = `"$rubySourcePath`"`n`$ironmon_fusion_predecessor_index_source_manifest_path = `"$rubySourceManifestPath`"`n$loaderCode`n$exporterCode"
...
Invoke-IronmonGameRuntime -GameRoot $resolvedGameRoot `
    ...
    -OperationName "fusion predecessor index extraction" `
...
if (-not (Test-Path -LiteralPath $resolvedOutputPath) -or
    ...
    throw "The game runtime did not produce a fusion predecessor index."
}
if (-not (Test-Path -LiteralPath "$resolvedOutputPath.summary")) {
    throw "The game runtime did not produce a fusion predecessor index summary."
}
$summary = @{}
...
foreach ($key in "signature_count", "membership_count", "normal_species_count") {
    if (-not $summary.ContainsKey($key) -or $summary[$key] -lt 1) {
        throw "The fusion predecessor index summary is missing '$key'."
    }
}
...
Write-Output "Fusion predecessor index generated from '$resolvedGameRoot': $($summary.signature_count) signatures, $($summary.membership_count) memberships."
