param(
    [string]$GameRoot = (Split-Path -Parent (Split-Path -Parent (Split-Path -Parent $PSScriptRoot))),
    [string]$OutputPath = (Join-Path (Split-Path -Parent (Split-Path -Parent $PSScriptRoot)) "data\fusion_predecessor_index.dat"),
    [ValidateRange(1, 600)]
    [int]$TimeoutSeconds = 90,
    [switch]$ShowGameWindow
)

$ErrorActionPreference = "Stop"
. (Join-Path $PSScriptRoot "GameRuntime-Tooling.ps1")

$projectRoot = Split-Path -Parent (Split-Path -Parent $PSScriptRoot)
$resolvedGameRoot = [IO.Path]::GetFullPath($GameRoot)
$resolvedProjectRoot = [IO.Path]::GetFullPath($projectRoot)
$resolvedOutputPath = [IO.Path]::GetFullPath($OutputPath)
$resolvedSourcePath = [IO.Path]::GetFullPath((Join-Path $projectRoot "src"))
$resolvedSourceManifestPath = [IO.Path]::GetFullPath((Join-Path $resolvedSourcePath "load_order.json"))
$loaderSourcePath = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot "Script-Loader.rb"))
$exporterSourcePath = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot "Export-FusionPredecessorIndex.rb"))
Assert-PathWithinDirectory -Path $resolvedOutputPath -Directory $resolvedProjectRoot
Assert-PathWithinDirectory -Path $resolvedSourcePath -Directory $resolvedGameRoot
Assert-PathWithinDirectory -Path $resolvedSourceManifestPath -Directory $resolvedSourcePath
Assert-PathWithinDirectory -Path $loaderSourcePath -Directory $resolvedGameRoot
Assert-PathWithinDirectory -Path $exporterSourcePath -Directory $resolvedGameRoot
foreach ($requiredPath in $loaderSourcePath, $exporterSourcePath, $resolvedSourceManifestPath) {
    if (-not (Test-Path -LiteralPath $requiredPath)) {
        throw "Required fusion predecessor index input was not found at '$requiredPath'."
    }
}

$rubyOutputPath = $resolvedOutputPath.Replace('\', '/')
$rubyGameRoot = $resolvedGameRoot.Replace('\', '/')
$rubySourcePath = $resolvedSourcePath.Replace('\', '/')
$rubySourceManifestPath = $resolvedSourceManifestPath.Replace('\', '/')
$bootstrapMarker = "$rubyOutputPath.bootstrap"
$loaderCode = [IO.File]::ReadAllText($loaderSourcePath, [Text.Encoding]::UTF8)
$exporterCode = [IO.File]::ReadAllText($exporterSourcePath, [Text.Encoding]::UTF8)
$bootstrapSource = "File.binwrite(`"$bootstrapMarker`", `"bootstrap loaded\n`")`n`$ironmon_fusion_predecessor_index_output_path = `"$rubyOutputPath`"`n`$ironmon_fusion_predecessor_index_game_root = `"$rubyGameRoot`"`n`$ironmon_fusion_predecessor_index_source_path = `"$rubySourcePath`"`n`$ironmon_fusion_predecessor_index_source_manifest_path = `"$rubySourceManifestPath`"`n$loaderCode`n$exporterCode"

New-Item -ItemType Directory -Force -Path (Split-Path -Parent $resolvedOutputPath) | Out-Null
foreach ($suffix in ".progress", ".error", ".bootstrap", ".summary", ".tmp") {
    Remove-Item -LiteralPath "$resolvedOutputPath$suffix" -Force -ErrorAction SilentlyContinue
}

Invoke-IronmonGameRuntime -GameRoot $resolvedGameRoot `
    -RubySource $bootstrapSource `
    -TimeoutSeconds $TimeoutSeconds `
    -OperationName "fusion predecessor index extraction" `
    -ErrorReportPath "$resolvedOutputPath.error" `
    -ShowGameWindow:$ShowGameWindow

if (-not (Test-Path -LiteralPath $resolvedOutputPath) -or
    (Get-Item -LiteralPath $resolvedOutputPath).Length -eq 0) {
    throw "The game runtime did not produce a fusion predecessor index."
}
if (-not (Test-Path -LiteralPath "$resolvedOutputPath.summary")) {
    throw "The game runtime did not produce a fusion predecessor index summary."
}
$summary = @{}
Get-Content -LiteralPath "$resolvedOutputPath.summary" | ForEach-Object {
    $parts = $_ -split '=', 2
    if ($parts.Count -eq 2) {
        $summary[$parts[0]] = [int]$parts[1]
    }
}
foreach ($key in "signature_count", "membership_count", "normal_species_count", "excluded_sprite_author_count") {
    if (-not $summary.ContainsKey($key) -or $summary[$key] -lt 1) {
        throw "The fusion predecessor index summary is missing '$key'."
    }
}
if (-not $summary.ContainsKey("rejected_autogenerated_credit_count") -or
    $summary["rejected_autogenerated_credit_count"] -lt 0) {
    throw "The fusion predecessor index summary is missing the autogenerated-credit audit."
}

foreach ($suffix in ".progress", ".error", ".bootstrap", ".summary", ".tmp") {
    Remove-Item -LiteralPath "$resolvedOutputPath$suffix" -Force -ErrorAction SilentlyContinue
}
Write-Output "Fusion predecessor index generated from '$resolvedGameRoot': $($summary.signature_count) signatures, $($summary.membership_count) memberships, $($summary.rejected_autogenerated_credit_count) autogenerated credit rows rejected."
