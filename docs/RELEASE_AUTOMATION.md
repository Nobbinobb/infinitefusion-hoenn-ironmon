...
This is the validated production workflow proposal on `feature/release-automation-test`. It replaces the earlier disposable rehearsal triggers. [Local validation](audits/RELEASE_AUTOMATION_FRESHNESS.md) and the completed [hosted orchestration rehearsal](audits/RELEASE_AUTOMATION_ORCHESTRATION.md) record the evidence. The private rehearsal verified required checks, candidate reuse, draft recovery, immutable publication, automatic full rebuilding after a simulated input change, and ordinary PR recovery. Production settings are prepared for adoption; production main, the real stable release and repository visibility remain unchanged.
...
1. **Release automation required checks** (`22383153`) is prepared **disabled**. It adds **PR validation** and **Release readiness**, both bound to GitHub Actions integration `15368`, with strict up-to-date enforcement and no bypass. The equivalent ruleset is active in the rehearsal repository. After the reviewed automation PR is merged and these workflows are present on main, change this production ruleset's enforcement to **Active**. Enabling it earlier would block existing PRs whose base does not yet contain the workflows. Do not enable merge queue without adding its `merge_group` triggers and validation policy.
...
4. Adopt through the reviewed automation PR without increasing the application version, then activate ruleset `22383153`. Scheduled/manual workflows become available on the default branch. This does not enable automatic PR merging or change repository visibility. Normal release PRs publish automatically only after acceptance.

Private-plan environment support accepts the main-only deployment policy but rejects reviewer/wait-timer protection fields, even when empty. Those fields are omitted; they are not needed for the requested automatic publication. Repository-wide mandatory Action SHA pinning remains off until adoption because the old main workflow still uses version tags; all Actions in this proposal are already pinned.
...
On 2026-09-06 GitHub rejected the contributor-approval settings endpoint with `Fork PR approval is not allowed for private repositories`. After an authorized visibility change, set the Actions fork approval policy to **all external contributors** (`all_external_contributors`) and read it back. This cannot be pre-enabled while private. Once the pinned workflows are adopted, repository-wide mandatory full-length Action SHA pinning can also be enabled. Neither setting requires changing the testing policy.
