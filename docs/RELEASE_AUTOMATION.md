...
Local validation can run `dotnet run --project tracker/tools/Ironmon.SpriteSizeTool -c Release -- game <output.json> <40-character-game-commit>` to measure without publishing. `Test-SpriteDownloadEstimates.ps1` exercises publication only against a fake GitHub API. The ordinary updater test build compiles the measurement tool and tests shared target selection, header-only reads, failed scans, feed bounds/freshness and selected totals.
