# Installing Ironmon 0.5.0

Ironmon `0.5.0` targets Pokemon Infinite Fusion 2 version 6.8.0.
...
3. Extract `Ironmon-v0.5.0-move-access.zip` into the game's root directory—the
...
before 0.5.0 retain native move access because they do not declare the new
generator metadata. Start a new run or use F7 to enable move-access
randomization. Existing supported ability and base-stat metadata continues to
...
entry also includes ability, base-stat, and move-access generator versions and
fingerprints. A log write failure never prevents the game from running.
